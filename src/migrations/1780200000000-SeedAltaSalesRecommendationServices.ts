  ...
  implements MigrationInterface
{
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const packageCategoryName = categories[0].name;

    for (const service of services) {
      const categoryId = categoryIds.get(service.category) ?? null;

      if (service.category === packageCategoryName) {
        const packageType = (service.skills[0] ?? service.category).slice(0, 50);
        ...
        await queryRunner.query(
          `
            INSERT INTO "service_package" (
              "name", "description", "tags", "packageType", "price", "categoryId", "deletedAt"
            )
