...
describe('AdminChatService', () => {
  describe('claim', () => {
    it('claims a session in awaiting status and posts an announcement message', async () => {
      ...
      // AdminChatService delegates to HandoffService which owns sessionRepository.
    ...
    it('throws NotFoundException when the session does not exist', async () => {
      ...
      hs.sessionRepository.findOne.mockResolvedValueOnce(null);
  ...
  describe('resolve', () => {
    it('resolves an in_progress session claimed by the same operator', async () => {
      ...
      expect(emittedEvents).toContain('chat:new_message');
      expect(
        savedMessages.some((m) => String(m.text).includes('покинул чат')),
      ).toBe(true);
    });
  ...
  describe('listOperatorSessions', () => {
    ...
    it('never lists expert-type sessions', async () => {
      const { service, sessionRepository } = buildService();
      sessionRepository.find.mockResolvedValue([]);
      await service.listOperatorSessions('active');
      expect(sessionRepository.find).toHaveBeenCalledWith(
        expect.objectContaining({
          where: expect.objectContaining({ type: ChatSessionType.Platform }),
        }),
      );
    });
  });
