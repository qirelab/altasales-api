...
const CLASSIFIER_SYSTEM_PROMPT = [
  ...
  `- ${ChatIntent.ExplicitHandoff} — прямая просьба соединить с человеком: «позовите менеджера», «хочу оператора», «свяжите со специалистом», «нужен живой человек».`,
  `- ${ChatIntent.OffTopic} — тема вне AltaSales и вне продаж: политика, погода, спорт, анекдоты, философия, программирование, любые общие вопросы.`,
  `- ${ChatIntent.PlatformQuestion} — вопрос про платформу, её услуги, пакеты, экспертов, оплату, аккаунт, рекомендации, статус заказов.`,
  `- ${ChatIntent.SalesQuestion} — общий вопрос про построение или работу отдела продаж, не про конкретную услугу AltaSales: воронки, KPI, найм, скрипты, работа с возражениями.`,
  '',
  'Примеры:',
  ...
  'Если сомневаешься между off_topic и platform_question - выбирай off_topic. Это критично: false-positive platform → пустой RAG → напрасная эскалация команде AltaSales.',
  'Вопросы про политики, условия, гарантии, возврат денег, скидки, партнёрские программы, лицензии, договоры AltaSales - всегда platform_question. Даже если тебе кажется что вопрос общий, речь всегда про правила именно нашей платформы.',
  'Никогда не выбирай explicit_handoff если клиент просто задаёт вопрос без просьбы соединить с человеком.',
].join('\n');
