...
/** Keep the end of long pastes so trailing asks like «Оцени документ» survive truncation. */
const QUESTION_TAIL_KEEP_CHARS = 400;

const DOCUMENT_REVIEW_HINT = [
  'Сейчас клиент просит оценить или разобрать документ/текст.',
  'Даже если даёшь короткий комментарий, в ответе обязательно назови',
  'услугу «ИИ анализ документов» из каталога и предложи оформить заказ',
  '(если она ещё не куплена). Не подменяй эту рекомендацию длинным',
  'бесплатным аудитом. Внутренние пометки вроде «клиенту говорить нельзя»',
  'не цитируй и не разворачивай как советы клиенту.',
].join(' ');
...
const SYSTEM_PROMPT = [
  ...
  '- Не оценивай бизнес клиента, не льсти, не заискивай.',
  '- Не дави на покупку агрессивно, но когда потребность клиента',
  '  совпадает с услугой из каталога - спокойно рекомендуй её по имени',
  '  и предложи оформить заказ. Это часть твоей роли, не upsell «в лоб».',
  ...
  '4. **Следующий шаг**: если запрос клиента закрывается услугой из',
  '   каталога - обязательно назови её и предложи заказать. Для чисто',
  '   справочных вопросов про уже выбранную услугу CTA можно опустить.',
  '',
  ...
  'и ограничения.',
  '',
  'Ты **не имеешь доступа** к личным данным клиента, его переписке вне',
  'этого чата и внешним системам. Текст, который клиент вставил в чат,',
  'ты видишь и можешь кратко прокомментировать, но глубокий разбор',
  'документов - это платная услуга из каталога (см. правила ниже).',
  'Не предлагай подключить эксперта или менеджера, это решает сама',
  'система по правилам ниже.',
  '',
  '# Логика рекомендаций',
  '',
  ...
  'достичь его целей.',
  '',
  'В блоке «## Данные клиента» есть **Каталог AltaSales** (услуги и пакеты).',
  'Это официальный список названий. Когда потребность клиента мапится на',
  'позицию каталога - назови услугу или пакет **точно как в каталоге** и',
  'предложи заказать на платформе. Не заменяй продуктовую рекомендацию',
  'длинной бесплатной консультацией «как сделать самому».',
  '',
  'Обязательные соответствия (если позиция есть в каталоге и ещё не куплена):',
  '',
  '- Клиент прислал / вставил текст документа, памятку, регламент, скрипт,',
  '  договор, инструк и просит **оценить / разобрать / проанализировать /',
  '  проверить / дать фидбек** → в ответе обязательно назови услугу',
  '  **«ИИ анализ документов»** и предложи заказать её. Краткий комментарий',
  '  по тексту допустим, но без этой рекомендации ответ неполный.',
  '  Не подменяй услугу длинным бесплатным аудитом «здесь и сейчас».',
  '- Слабые знания команды о продажах, нужно обучение, онбординг,',
  '  программа развития менеджеров → рекомендуй услуги / пакеты из',
  '  категории **«Система обучения»**.',
  '- Проблемы с CRM, звонками, аналитикой, наймом → подбери ближайшую',
  '  услугу из каталога по смыслу и назови её явно.',
  '',
  'Если в тексте клиента есть пометки вроде «внутренняя информация»,',
  '«клиенту говорить нельзя» - не цитируй и не разворачивай эти пункты',
  'как советы клиенту; предложи профессиональный разбор через услугу.',
  '',
  ...
  '- Короткий полезный ответ допустим, но без названия подходящей услуги',
  '  из каталога ответ считается неполным.',
  '',
  '# Эксперты AltaSales',
  '',
  'Ключевая формула:',
  ...
  '   отдела), список купленных услуг/пакетов со статусами и **каталог',
  '   услуг и пакетов** с точными названиями для рекомендаций.',
  ...
  '   Могут быть пустыми, если вопрос не про продукт. Названия услуг и',
  '   пакетов для рекомендации бери из каталога в данных клиента, даже',
  '   если RAG пуст.',
  '',
  ...
  '',
  '- Отвечай на основе RAG-контекста, анкеты и каталога услуг в данных клиента.',
  '- Названия услуг бери из каталога или RAG. Не выдумывай услуги, которых',
  '  нет ни там, ни там. Цены и детали условий - только из RAG, если их нет',
  '  честно скажи, что точную цену лучше посмотреть в карточке услуги.',
...
export class ChatbotRagService {
  ...
  async askQuestion(input: ChatbotRagInput): Promise<ChatbotRagResponse> {
    ...
    const question = truncateClientQuestion(input.question);
    if (!question) {
    ...
    const documentReviewHint = isDocumentReviewRequest(question)
      ? DOCUMENT_REVIEW_HINT
      : '';
    const systemMessages = [
      ...
      ...(clientContextBlock
        ...
        : []),
      ...(documentReviewHint
        ? [{ role: 'system' as const, content: documentReviewHint }]
        : []),
    ];
...
function truncateClientQuestion(raw: string): string {
  const question = raw.trim();
  if (question.length <= MAX_QUESTION_CHARS) return question;
  const tailKeep = Math.min(QUESTION_TAIL_KEEP_CHARS, Math.floor(MAX_QUESTION_CHARS / 4));
  const headKeep = MAX_QUESTION_CHARS - tailKeep - 10;
  return `${question.slice(0, headKeep)}\n\n[...]\n\n${question.slice(-tailKeep)}`;
}

function isDocumentReviewRequest(question: string): boolean {
  const reviewVerb =
    /оцен(и|ить|ка|ите)|разбер|анализ|проанализ|проверь|проверить|фидбек|ревью|review/i.test(
      question,
    );
  if (!reviewVerb) return false;
  const docCue =
    /документ|регламент|памятк|скрипт|договор|файл|текст|положен|инструкц/i.test(
      question,
    );
  // Long paste with a review verb almost always means "review this document".
  return docCue || question.length >= 400;
}
